...
class PartnerController extends GetxController{
    ...
    if(!isAdmin){
      ...
      FirebaseFirestore.instance.collection("direct-selling-referral").get().then((value) {
        value.docs.forEach((element) {
          ...
          FirebaseFirestore.instance.collection("direct-selling-referral").doc(element.id).
          collection("know-more").get().then((value) {
            List<KnowMore> knowMore = <KnowMore>[];
            value.docs.forEach((element) {
              knowMore.add(KnowMore.fromJson(element.data()));
              knowMore.sort((a,b)=>a.index.compareTo(b.index));
              referModel.knowMore = knowMore;
            });
          });
          FirebaseFirestore.instance.collection("direct-selling-referral").doc(element.id).
          collection("how-to-earn").get().then((value) {
            List<HowToEarn> howToEarn = <HowToEarn>[];
            value.docs.forEach((element) {
              howToEarn.add(HowToEarn.fromJson(element.data()));
              howToEarn.sort((a,b)=>a.index.compareTo(b.index));
              referModel.howToEarn = howToEarn;
            });
          });

          FirebaseFirestore.instance.collection("direct-selling-referral").doc(element.id).
          collection("about").get().then((value) {
            List<About> about = <About>[];
            value.docs.forEach((element) {
              about.add(About.fromJson(element.data()));
              about.sort((a,b)=>a.index.compareTo(b.index));
              referModel.about = about;
  ...
  void updatePartner(ReferModel refer) {
    ...
    try{
      FirebaseFirestore.instance.collection("direct-selling-referral").doc(refer.key).update(refer.toJson()).then((value) {
        ...
        /*FirebaseFirestore.instance.collection("direct-selling-referral").doc(refer.key).collection("about").doc().update(refer.about).then((value) {
  ...
  var isUpdate = true.obs;
  bool get getIsUpdate => isUpdate.value;
  set setIsUpdate(bool val){
    isUpdate.value = val;
    isUpdate.refresh();
  }
  var updateIndex = 0.obs;
  int get getUpdateIndex => updateIndex.value;
  set setUpdateIndex(int val){
    updateIndex.value = val;
    updateIndex.refresh();
  }
